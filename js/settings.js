...
function loadSettingsState() {
  ...
  if (intensityEl)
    intensityEl.textContent = intensityLabels[_wxIntensity] || "中";
...
function saveMinimaxConfig() {
  SettingStore.set(
    "minimaxKey",
    document.getElementById("minimaxKey").value.trim(),
  );
  SettingStore.set(
    "minimaxGroupId",
    document.getElementById("minimaxGroupId").value.trim(),
  );
  SettingStore.set(
    "minimaxVoice",
    document.getElementById("minimaxVoice").value.trim(),
  );
  SettingStore.set(
    "minimaxSpeed",
    document.getElementById("minimaxSpeed").value,
  );
  saToast("语音配置已保存");
}
...
async function previewMinimaxVoice() {
  ...
  if (!cfg.key) {
    saToast("请先填写并保存 MiniMax API Key", "error");
    return;
  }
  if (!cfg.groupId) {
    saToast("请先填写并保存 Group ID", "error");
    return;
  }
  if (!cfg.voice) {
    saToast("请先填写并保存 Voice ID", "error");
    return;
  }
  if (!text) {
    saToast("请输入试听文字", "error");
    return;
  }
...
function previewWpStyle() {
  ...
  document.getElementById("wpFrostVal").textContent =
    document.getElementById("wpFrost")?.value || "0";
...
function selectWpFit(fit) {
  _wpFit = fit;
  document
    .querySelectorAll(".wp-fit-btn")
    .forEach((b) => b.classList.remove("active"));
  document
    .querySelector(`.wp-fit-btn[data-fit="${fit}"]`)
  ...
  if (layer) {
    ...
    if (!pseudo) {
      ...
      pseudo.style.cssText =
        "position:absolute;inset:0;pointer-events:none;z-index:1;transition:opacity .3s;";
      ...
      if (dataUrl) {
        ...
        if (layer) {
          ...
          layer.style.backgroundRepeat =
            style.fit === "tile" ? "repeat" : "no-repeat";
          layer.style.backgroundColor = "transparent";
...
function _addToGalleryItem(dataUrl, name) {
  ...
  if (meta.length > WP_GALLERY_MAX) {
    ...
    removed.forEach((m) => {
      if (typeof ImgDB !== "undefined")
        ImgDB.remove(WP_GALLERY_IDB_PREFIX + m.id).catch(() => {});
    });
  }
...
function renderWpGallery() {
  ...
  Array.from(grid.children).forEach((c) => {
    if (c.id !== "wpGalleryEmpty") c.remove();
  });
  ...
  meta.forEach((m) => {
    ...
    /* 点击选用 */
    thumb
      .querySelector(".wp-gallery-img-wrap")
...
function updateWxIntensityLabel(val) {
  ...
  const labels = ["", "轻", "中", "强"];
  document.getElementById("wxIntensityVal").textContent =
    labels[_wxIntensity] || "中";
...
function _startWeatherEffect(type) {
  /* ① 停止旧动画 + 清除雷暴定时器 */
  if (_wxAnimId) {
    cancelAnimationFrame(_wxAnimId);
    _wxAnimId = null;
  }
  if (window._wxThunderTimer) {
    clearTimeout(window._wxThunderTimer);
    window._wxThunderTimer = null;
  }
  _wxParticles = [];
  ...
  requestAnimationFrame(() => {
    ...
    const parent =
      document.getElementById("homeScreen") || canvas.parentElement;
      ...
      window._wxThunderTimer = setTimeout(
        () => {
          ...
          if (!flash) {
            ...
            flash.style.cssText =
              "position:absolute;inset:0;z-index:9;pointer-events:none;background:rgba(255,255,255,0);transition:background .05s;";
...
function _rebuildScreenFilter() {
  ...
  const brightness = SettingStore.get("brightness", 100);
  const filter =
    SettingStore.get("_tmpFilter", null) ||
    SettingStore.get("screenFilter", "none");
  const eyeCare = SettingStore.get("eyeCare", false);
  const nightMode = SettingStore.get("nightMode", false);

  let parts = [];
  if (brightness !== 100) parts.push(`brightness(${brightness / 100})`);
  if (filter && filter !== "none" && FILTER_PRESETS[filter])
    parts.push(FILTER_PRESETS[filter]);
...
function toggleDynamicIsland(hide) {
  ...
  if (el) {
    ...
    // 隐藏时同时清除占位高度，避免留空黑条
    el.style.margin = hide ? "0" : "";
    el.style.height = hide ? "0" : "";
    el.style.overflow = hide ? "hidden" : "";
  }
...
function _applyScreenPos(pos, save) {
  ...
  if (info)
    info.textContent = `偏移：X ${pos.x >= 0 ? "+" : ""}${pos.x}px  Y ${pos.y >= 0 ? "+" : ""}${pos.y}px`;
...
function saveClockWidgetConfig() {
  const tz =
    document.querySelector("#tzGrid .sc-tz-item.active")?.dataset.tz || "local";
...
const IC_APPS = [
  {
    id: "voice",
    name: "心声",
    sel: "[onclick=\"openApp('voice')\"]",
    defaultSvg:
      '<path d="M16 4a5 5 0 015 5v7a5 5 0 01-10 0V9a5 5 0 015-5z"/><path d="M7 18a9 9 0 0018 0"/><line x1="16" y1="27" x2="16" y2="30"/><line x1="12" y1="30" x2="20" y2="30"/>',
    lineSvgs: {
      default:
        '<path d="M16 4a5 5 0 015 5v7a5 5 0 01-10 0V9a5 5 0 015-5z"/><path d="M7 18a9 9 0 0018 0"/><line x1="16" y1="27" x2="16" y2="30"/><line x1="12" y1="30" x2="20" y2="30"/>',
      ...
      music:
        '<path d="M12 26V12l14-3v14"/><circle cx="9" cy="26" r="3"/><circle cx="23" cy="23" r="3"/>',
    },
  },
  {
    id: "forum",
    name: "论坛",
    sel: "[onclick=\"openApp('forum')\"]",
    defaultSvg:
      '<rect x="4" y="4" width="24" height="18" rx="2"/><line x1="8" y1="11" x2="24" y2="11"/><line x1="8" y1="15" x2="20" y2="15"/><line x1="8" y1="19" x2="17" y2="19"/>',
    lineSvgs: {
      default:
        '<rect x="4" y="4" width="24" height="18" rx="2"/><line x1="8" y1="11" x2="24" y2="11"/><line x1="8" y1="15" x2="20" y2="15"/><line x1="8" y1="19" x2="17" y2="19"/>',
      bubble:
        '<path d="M4 6h24v16a2 2 0 01-2 2H8l-4 4V8a2 2 0 012-2z"/><circle cx="11" cy="15" r="1.5" fill="currentColor" stroke="none"/><circle cx="16" cy="15" r="1.5" fill="currentColor" stroke="none"/><circle cx="21" cy="15" r="1.5" fill="currentColor" stroke="none"/>',
  ...
  },
  {
    id: "diary",
    name: "小芽日记",
    sel: "[onclick=\"openApp('diary')\"]",
    defaultSvg:
      '<rect x="5" y="3" width="18" height="26" rx="2"/><path d="M5 3h3a2 2 0 012 2v22a2 2 0 01-2 2H5" stroke-width="1.4"/><line x1="13" y1="10" x2="20" y2="10"/><line x1="13" y1="15" x2="20" y2="15"/><line x1="13" y1="20" x2="18" y2="20"/>',
    lineSvgs: {
      default:
        '<rect x="5" y="3" width="18" height="26" rx="2"/><path d="M5 3h3a2 2 0 012 2v22a2 2 0 01-2 2H5" stroke-width="1.4"/><line x1="13" y1="10" x2="20" y2="10"/><line x1="13" y1="15" x2="20" y2="15"/><line x1="13" y1="20" x2="18" y2="20"/>',
      flower:
        '<circle cx="16" cy="16" r="4"/><circle cx="16" cy="8" r="3"/><circle cx="16" cy="24" r="3"/><circle cx="8" cy="16" r="3"/><circle cx="24" cy="16" r="3"/>',
  ...
  },
  {
    id: "street",
    name: "街の声",
    sel: "[onclick=\"openApp('street')\"]",
    defaultSvg:
      '<path d="M16 6a5 5 0 015 5c0 4-5 10-5 10S11 15 11 11a5 5 0 015-5z"/><circle cx="16" cy="11" r="2" fill="currentColor" stroke="none"/><path d="M6 28l3-8h14l3 8"/>',
    lineSvgs: {
      default:
        '<path d="M16 6a5 5 0 015 5c0 4-5 10-5 10S11 15 11 11a5 5 0 015-5z"/><circle cx="16" cy="11" r="2" fill="currentColor" stroke="none"/><path d="M6 28l3-8h14l3 8"/>',
      compass:
        '<circle cx="16" cy="16" r="10"/><polygon points="16,8 18,14 16,16 14,14"/><polygon points="16,24 14,18 16,16 18,18"/>',
  ...
  },
  {
    id: "candy",
    name: "糖果铺",
    sel: "[onclick=\"openApp('candy')\"]",
    defaultSvg:
      '<circle cx="16" cy="16" r="7"/><path d="M9 9l-4-4M23 9l4-4M9 23l-4 4M23 23l4 4"/><circle cx="16" cy="16" r="2.5" fill="currentColor" stroke="none"/>',
    lineSvgs: {
      default:
        '<circle cx="16" cy="16" r="7"/><path d="M9 9l-4-4M23 9l4-4M9 23l-4 4M23 23l4 4"/><circle cx="16" cy="16" r="2.5" fill="currentColor" stroke="none"/>',
      candy:
        '<path d="M10 22C10 22 8 14 16 14S22 22 22 22"/><path d="M12 10c0-2.2 1.8-4 4-4s4 1.8 4 4-1.8 3-4 3-4-.8-4-3z"/><path d="M10 22l2 5M22 22l-2 5"/>',
      star4:
        '<polygon points="16,4 18,14 28,16 18,18 16,28 14,18 4,16 14,14"/>',
  ...
  },
  {
    ...
    lineSvgs: {
      ...
      headphone:
        '<path d="M6 16a10 10 0 0120 0"/><rect x="3" y="15" width="5" height="8" rx="2"/><rect x="20" y="15" width="5" height="8" rx="2"/>',
    },
  },
  {
    id: "chat",
    name: "聊天",
    sel: "[onclick=\"openApp('chat')\"]",
    defaultSvg:
      '<path d="M4 6h24v16a2 2 0 01-2 2H8l-4 4V8a2 2 0 012-2z"/><circle cx="11" cy="14" r="1.5" fill="currentColor" stroke="none"/><circle cx="16" cy="14" r="1.5" fill="currentColor" stroke="none"/><circle cx="21" cy="14" r="1.5" fill="currentColor" stroke="none"/>',
    lineSvgs: {
      default:
        '<path d="M4 6h24v16a2 2 0 01-2 2H8l-4 4V8a2 2 0 012-2z"/><circle cx="11" cy="14" r="1.5" fill="currentColor" stroke="none"/><circle cx="16" cy="14" r="1.5" fill="currentColor" stroke="none"/><circle cx="21" cy="14" r="1.5" fill="currentColor" stroke="none"/>',
      heart:
        '<path d="M4 6h24v14H8l-4 4V8a2 2 0 012-2z"/><path d="M16 18s-4-3-4-6a4 4 0 018 0c0 3-4 6-4 6z"/>',
      love: '<path d="M16 26s-10-7-10-14a10 10 0 0120 0c0 7-10 14-10 14z"/>',
      phone:
        '<path d="M6 4l4 2-2 4 3 3 4-2 2 4-5 3C8 17 5 11 6 4z"/><path d="M18 4a10 10 0 018 8"/><path d="M18 9a5 5 0 014 4"/>',
    },
  },
  {
    id: "worldbook",
    name: "世界书",
    sel: "[onclick=\"openApp('worldbook')\"]",
    defaultSvg:
      '<circle cx="16" cy="16" r="11"/><path d="M5 16h22M16 5c-3 3-5 7-5 11s2 8 5 11M16 5c3 3 5 7 5 11s-2 8-5 11"/>',
    lineSvgs: {
      default:
        '<circle cx="16" cy="16" r="11"/><path d="M5 16h22M16 5c-3 3-5 7-5 11s2 8 5 11M16 5c3 3 5 7 5 11s-2 8-5 11"/>',
      ...
      compass2:
        '<circle cx="16" cy="16" r="11"/><circle cx="16" cy="16" r="3"/><line x1="16" y1="5" x2="16" y2="13"/><line x1="16" y1="19" x2="16" y2="27"/>',
      planet:
        '<circle cx="16" cy="16" r="8"/><ellipse cx="16" cy="16" rx="14" ry="5"/>',
    },
  },
  {
    id: "settings",
    name: "设置",
    sel: "[onclick=\"openApp('settings')\"]",
    defaultSvg:
      '<circle cx="16" cy="16" r="3.5"/><path d="M16 4v3M16 25v3M4 16h3M25 16h3M7.5 7.5l2.1 2.1M22.4 22.4l2.1 2.1M7.5 24.5l2.1-2.1M22.4 9.6l2.1-2.1"/>',
    lineSvgs: {
      default:
        '<circle cx="16" cy="16" r="3.5"/><path d="M16 4v3M16 25v3M4 16h3M25 16h3M7.5 7.5l2.1 2.1M22.4 22.4l2.1 2.1M7.5 24.5l2.1-2.1M22.4 9.6l2.1-2.1"/>',
      ...
      diamond:
        '<rect x="8" y="8" width="16" height="16" rx="2" transform="rotate(45 16 16)"/><circle cx="16" cy="16" r="3"/>',
    },
  },
];
...
const IC_STYLE_CONFIGS = {
  default: {
    ...
    border: "1px solid rgba(220,220,218,0.5)",
    borderRadius: "50%",
    svgColor: "#444",
    strokeW: "1.4",
    fill: "none",
  ...
  ins: {
    bg: "linear-gradient(135deg,rgba(255,220,230,0.6),rgba(220,200,255,0.6))",
  ...
  korean: {
    bg: "rgba(245,240,255,0.8)",
    border: "1px solid rgba(220,210,240,0.5)",
    borderRadius: "40%",
    svgColor: "#9070c0",
    strokeW: "1.4",
    fill: "none",
...
function applyIconBgToPage(bg, save) {
  ...
  document.querySelectorAll(".app-icon, .dock-icon").forEach((icon) => {
    if (bg === "glass") {
      ...
      icon.style.background = cfg.bg;
      icon.style.border =
        bg === "none" ? "none" : "1px solid rgba(200,200,198,0.3)";
    }
    if (cfg.color)
      icon.querySelectorAll("svg").forEach((s) => (s.style.color = cfg.color));
...
function applyIconLine(appId, lineKey) {
  ...
  const iconEls = document.querySelectorAll(
    `.app-icon${app.sel}, .dock-icon${app.sel}`,
  );
...
function renderIconCustomList() {
  ...
  IC_APPS.forEach((app) => {
    ...
    /* 加载已保存图片 */
    if (typeof ImgDB !== "undefined") {
      ImgDB.get(IC_IMG_IDB_PREFIX + app.id)
        .then((dataUrl) => {
...
function _applyCustomIconImg(appId, dataUrl) {
  ...
  const def = document.querySelector(
    `#icCustomWrap_${appId} .ic-custom-icon-default`,
  );
  if (img) {
    img.src = dataUrl;
...
function resetIconImg(appId) {
  ...
  if (typeof ImgDB !== "undefined")
    ImgDB.remove(IC_IMG_IDB_PREFIX + appId).catch(() => {});
  saToast("已恢复默认图标");
}
...
function _addToIconGalleryItem(dataUrl, name) {
  ...
  if (meta.length > IC_GALLERY_MAX) {
    meta.splice(IC_GALLERY_MAX).forEach((m) => {
      if (typeof ImgDB !== "undefined")
        ImgDB.remove(IC_GALLERY_IDB_PREFIX + m.id).catch(() => {});
    });
  }
  localStorage.setItem(IC_GALLERY_KEY, JSON.stringify(meta));
  if (typeof ImgDB !== "undefined")
    ImgDB.set(IC_GALLERY_IDB_PREFIX + id, dataUrl).catch(() => {});
  renderIconGallery();
}
...
function renderIconGallery() {
  ...
  if (empty) empty.style.display = "none";
  Array.from(grid.children).forEach((c) => {
    if (c.id !== "icGalleryEmpty") c.remove();
  });
...
function _showIconTargetPicker(galleryId, idbKey) {
  ...
  picker.style.cssText =
    "position:fixed;inset:0;z-index:9999;display:flex;align-items:flex-end;background:rgba(0,0,0,0.4);backdrop-filter:blur(4px);";
  picker.innerHTML = `
...
function deleteIconGalleryItem(e, id) {
  ...
  localStorage.setItem(IC_GALLERY_KEY, JSON.stringify(meta));
  if (typeof ImgDB !== "undefined")
    ImgDB.remove(IC_GALLERY_IDB_PREFIX + id).catch(() => {});
  renderIconGallery();
}
...
function applyFontToPage(fontKeyOrFamily, save) {
  ...
  const family = isBuiltin
    ? FONT_MAP[fontKeyOrFamily]
    : `'${fontKeyOrFamily}',sans-serif`;
...
function _persistFontStyle(fontKeyOrFamily, family) {
  ...
  try {
    localStorage.setItem(
      "xxj_set_fontInjectCSS",
      `body,#phoneScreen,#settingsApp,*{font-family:${family}!important;}`,
    );
  } catch (e) {}
}
...
async function importFontFromUrl() {
  ...
  const name =
    document.getElementById("ftUrlName")?.value?.trim() ||
    _guessNameFromUrl(url);
  if (!url) {
    saToast("请填写字体 URL", "error");
    return;
  }
...
async function previewFontUrl() {
  const url = document.getElementById("ftUrlInput")?.value?.trim();
  const name =
    document.getElementById("ftUrlName")?.value?.trim() || "PreviewFont";
  if (!url) {
    saToast("请先填写字体 URL", "error");
    return;
  }
  saToast("加载预览中…");
  try {
...
function importFontFromFile(event) {
  ...
  if (info)
    info.textContent = `${file.name} · ${(file.size / 1024).toFixed(0)} KB — 加载中…`;
  ...
  reader.onload = async (e) => {
    ...
    try {
      ...
      _addToFontLib({
        ...
        name,
        family,
        source: dataUrl.slice(0, 60),
        type: "file",
        idbKey: FT_IDB_PREFIX + id,
        ts: Date.now(),
      });
...
async function applyFontFromLib(family, type, idbKey, encodedSource) {
  ...
  try {
    ...
    const already = [...document.fonts].some(
      (f) => f.family === family || f.family === `"${family}"`,
    );
    if (!already) {
      /* 需要重新加载 */
...
function _restoreFontOnLoad() {
  ...
  if (currentMeta) {
    if (currentMeta.type === "url") {
      ...
      currentMeta.type === "file" &&
      currentMeta.idbKey &&
      typeof ImgDB !== "undefined"
    ) {
      /* 文件字体需要异步加载 DataURL */
      ImgDB.get(currentMeta.idbKey)
        .then((dataUrl) => {
          if (dataUrl)
            _loadFontFace(currentMeta.family, dataUrl)
              .then(() => {
...
function _getAllIdbKeys() {
  ...
  ftLib.forEach((m) => {
    if (m.idbKey) base.push(m.idbKey);
  });
  return base;
}
...
function calcStorage() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (k && k.startsWith("xxj_"))
      textBytes += (localStorage.getItem(k) || "").length * 2;
  }
  ...
  if (typeof ImgDB !== "undefined") {
    ...
    Promise.all(idbKeys.map((k) => ImgDB.get(k).catch(() => null)))
      .then((results) => {
        let imgBytes = 0,
          ftBytes = 0;
        results.forEach((r, i) => {
...
function importData(event) {
  ...
  reader.onload = async (e) => {
    try {
      ...
      /* IndexedDB */
      if (typeof ImgDB !== "undefined") {
        const idbPairs = Object.entries(data).filter(([k]) =>
          k.startsWith("idb_"),
        );
        await Promise.all(
          idbPairs.map(([k, v]) =>
...
function saToast(msg, type = "success") {
  ...
  Object.assign(el.style, {
    ...
    background:
      type === "error" ? "rgba(180,50,50,0.92)" : "rgba(42,42,42,0.88)",
    color: "#fff",
    padding: "9px 18px",
    borderRadius: "20px",
    fontSize: "13px",
